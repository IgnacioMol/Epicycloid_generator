...
- **Pasada de estilo (imitar el TFG de referencia):** reescritas las descripciones de funciones y componentes del cap. 6 con el registro narrativo de UrbanGuardian — fórmulas «Este fragmento de código implementa…», «El método X() se encarga de…», «La función Y() permite…», conectores encadenados (Primero… A continuación… Por último…) y cierres «Esta lógica/estrategia permite…». Se mantuvieron encabezados, bloques de código y figuras. Se añadió «(RF7)» al export/import JSON (Figura 6.28) para cuadrar con la trazabilidad del cap. 4 (RF7 → CU9/CU10/CU14).
